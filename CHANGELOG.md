...
- A `Duplicates` status filter, showing every tab whose URL is open in more than one tab. Matching
  is exact, so URLs differing only by fragment or query are treated as different pages; the filter
  is meant to reveal clutter, not to hide tabs behind a guess. Every copy is listed, not just the
  redundant ones, so it is clear which is which before closing any of them.
...
[0.3.0]: https://github.com/ajunior/tabsheet/compare/v0.2.0...v0.3.0
